      ...
      <body className={inter.className}>
        <AppProvider {...{ isMenuOpen, setIsMenuOpen }}>
          <div className="flex min-h-screen text-white">
            <aside
              className={`fixed top-0 left-0 h-full bg-black text-white transition-all duration-300 ${
                isMenuOpen ? "w-64" : "w-0"
              } md:w-64`}
            >
              <NavMenu isMenuOpen={isMenuOpen} />
            </aside>
            <main className="flex-1 ml-0 md:ml-64 bg-[#1a1a1e]">
              <header className="p-4 bg-[#2b2a2aaa]">
                {!isMenuOpen && (
                  <button
